...
The concrete surface is LedgerDesk, a local synthetic banking application with server-rendered tables and a named iframe. The app profile supplies locators, permissions, and condition detectors, not action order. This deliberately trades open-ended exploration for a small auditable control vocabulary. A Node.js runtime owns the browser and local operator server. During discovery it invokes a Python worker for OpenAI, Anthropic, or compatible model APIs through a bounded JSON stdin/stdout protocol. Python normalizes responses; TypeScript validates decisions and enforces policy. Replay starts no Python process. JSON files keep persistence easy to inspect. The sandbox has no data API used by automation.

The main hypothesis is that a capability can preserve business correctness across presentation differences while failing closed on semantic uncertainty. The [assurance lab](evidence/live/replay-corpus/SUMMARY.md) exercises 15 cases across two presentations. [Genuine Claude API discovery](evidence/live/README.md) completed in seven model calls; the resulting capability passed all 15 replay cases without model credentials.
...
Next: assess discovery consistency across more goals, conduct an operator-led usability walkthrough, then challenge the adapter against an independently built legacy application. Identity checks cannot detect an application lying consistently about both identifier and balance; write workflows also need application-level idempotency and reconciliation. See [decision notes](docs/DECISIONS.md) for alternatives and falsification criteria. Genuine discovery, linked replay, and scripted same-session handoff evidence are included.
