...
import { loadProfile, fixture } from '../tests/fixture.js';
...
await writeFile('evidence/offline/authored-artifact.json',JSON.stringify(artifact,null,2)+'\n');
const runs: unknown[] = [];
try {
  for (const [scenario,memberId] of [['normal','67890'],['normal','99999'],['transient','12345'],['denied','12345'],['expired','12345']]) {
    ...
    } finally { await runner.handoff.close(); await surface.close(); }
  }
  await writeFile('evidence/offline/index.json',JSON.stringify({kind:'authored_fixture_replay',note:'These are real browser replay runs of an authored fixture. No LLM discovery is claimed.',runs},null,2)+'\n');
