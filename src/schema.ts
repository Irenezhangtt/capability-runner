...
}).strict().refine(t => t.by !== 'role' || !!t.role, 'Role targets require a role');
...
}).strict().refine(d => d.done ? !d.action : !!d.action, 'Choose exactly one action or done');
...
export function validateInputs(capability: Pick<Capability, 'inputs'>, values: Record<string, unknown>) {
  if (Object.keys(values).some(k => !(k in capability.inputs))) throw new RunError('INVALID_INPUT', 'Unknown input parameter');
