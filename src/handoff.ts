...
export class Handoff {
  ...
  constructor(
    private surface: ManagedSurface,
    ...
    this.server = createServer(async (req, res) => {
      ...
      if (req.method === 'POST' && this.pending) {
        ...
        if (action === '/take' && this.surface.owner === 'paused') {
          ...
          await this.surface.evidence.event('control_transferred', {
            from: 'paused',
            to: 'human',
          ...
          res.writeHead(409).end('Invalid control transition');
          return;
        }
  ...
  async request(request: Intervention, restored: () => Promise<boolean>) {
    ...
    if (!this.enabled)
      throw new RunError(
        ...
        'Rerun headed with --human for live session handoff',
      );
    ...
    if (!resume)
      throw new RunError('INTERVENTION_ABORTED', 'Operator aborted or handoff timed out');
    ...
    if (!verified)
      throw new RunError(
        'RESUME_CHECK_FAILED',
        'Operator returned control outside the expected state',
      );
