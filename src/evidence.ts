...
export class Evidence {
  ...
  constructor(root: string, values: unknown[] = []) {
    ...
    this.secrets = values.filter((v): v is string | number => typeof v === 'string' || typeof v === 'number').map(String).filter(Boolean);
  }
  protect(value: string | number) { this.secrets.push(String(value)); }
  redact(text: string) {
    let result = text;
    for (const secret of [...this.secrets].sort((a,b) => b.length-a.length)) result = result.split(secret).join('[REDACTED]');
    return result.replace(/\b(?:sk-|Bearer\s+)[A-Za-z0-9_-]+/gi, '[REDACTED]')
  ...
  async event(type: string, data: Record<string, unknown> = {}) {
    await this.init();
    await appendFile(join(this.dir, 'events.jsonl'), JSON.stringify(this.sanitize({seq: ++this.sequence, at: new Date().toISOString(), type, ...data}))+'\n', {mode:0o600});
  }
