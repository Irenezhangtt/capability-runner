...
export class ApiPlanner implements Planner {
  ...
  constructor(
    ...
    if (!['openai', 'anthropic', 'compatible'].includes(this.provider))
      throw new RunError('MODEL_CONFIG', 'Unknown LLM_PROVIDER');
    this.key = (this.provider === 'anthropic' ? env.ANTHROPIC_API_KEY : env.OPENAI_API_KEY) || '';
    if (!this.key || !this.model)
      throw new RunError('MODEL_CONFIG', 'Configure LLM_MODEL and the provider API key in .env');
    this.endpoint =
      this.provider === 'anthropic'
        ? 'https://api.anthropic.com/v1/messages'
        : `${(env.LLM_BASE_URL || 'https://api.openai.com/v1').replace(/\/$/, '')}/${this.provider === 'compatible' ? 'chat/completions' : 'responses'}`;
    const url = new URL(this.endpoint);
    if (url.protocol !== 'https:' || url.username || url.password || url.search)
      throw new RunError(
        'MODEL_CONFIG',
        'Provider endpoint must use HTTPS without embedded credentials or query parameters',
      );
  ...
  async decide(context: Record<string, unknown>): Promise<DecisionSpec> {
    ...
    try {
      ...
      throw new RunError('MODEL_UNAVAILABLE', 'Model request failed or timed out');
    }
    // Never log provider errors: they may echo request content or credentials.
    if (!response.ok)
      throw new RunError('MODEL_HTTP_ERROR', `Model provider returned HTTP ${response.status}`);
    ...
    try {
      ...
      throw new RunError(
        ...
        'Decision did not match the bounded action schema',
      );
    }
  }
}
