...
export interface Surface {
  ...
  act(action: ActionSpec, inputs: Record<string, unknown>, outputs: Capability['outputs']): Promise<string | number | undefined>;
...
export class BrowserSurface implements ManagedSurface {
  ...
  constructor(readonly profile: AppProfile, readonly evidence: Evidence) { this.policy = new Policy(profile); }
  async launch(headed = false, scenario = 'normal') {
    this.browser = await chromium.launch({ headless: !headed, channel: process.env.BROWSER_CHANNEL || undefined });
    ...
    await this.context.route('**/*', async route => {
      try {
        ...
        if (!['GET','HEAD'].includes(route.request().method()) && !(this.owner === 'human' && new URL(route.request().url()).pathname === '/legacy/restore')) {
          throw new RunError('POLICY_BLOCKED', 'Write requests are disabled');
        }
        await route.continue();
      } catch { this.violation = 'POLICY_BLOCKED'; await route.abort('blockedbyclient'); }
    });
    await this.context.routeWebSocket('**/*', socket => socket.close());
    await this.context.addCookies([{name:'scenario',value:scenario,url:this.profile.origin}]);
    await this.context.exposeBinding('__auditHuman', async (_source, event: {event:string; tag:string; control?:string}) => {
      if (this.owner !== 'human') return;
      const controls = Object.values(this.profile.targets).map(t => t.name).concat('Restore training session');
      await this.evidence.event('human_action', {event: ['click','input','change','keydown'].includes(event.event) ? event.event : 'other', tag: /^[A-Z]{1,12}$/.test(event.tag) ? event.tag : 'unknown', control: controls.includes(event.control ?? '') ? event.control : '[unrecognized control]', values: '[not captured]'});
    });
    await this.context.addInitScript(() => {
      for (const event of ['click','input','change','keydown']) document.addEventListener(event, e => {
        const el = e.target as HTMLElement | null;
        if (!el) return;
        const control = el.getAttribute('aria-label') ?? (el instanceof HTMLInputElement ? el.labels?.[0]?.textContent : el.textContent)?.trim();
        const audit = (window as unknown as {__auditHuman:(e:unknown)=>Promise<void>}).__auditHuman;
    ...
    this.context.on('page', async page => { if (page !== this.page) { this.violation = 'UNEXPECTED_POPUP'; await page.close(); } });
    this.page.on('dialog', async dialog => {
      this.violation = 'UNEXPECTED_DIALOG';
      await dialog.dismiss();
    });
    this.page.on('download', async download => { this.violation = 'UNEXPECTED_DOWNLOAD'; await download.cancel(); });
  }
  ...
  async healthy() {
    if (this.violation) throw new RunError(this.violation, 'Browser boundary or unexpected UI event stopped the run');
    this.policy.url(this.page.url());
    if (!await this.page.getByText(this.profile.fingerprint, {exact:true}).isVisible()) throw new RunError('APP_VERSION_MISMATCH', 'Expected app fingerprint is absent');
  }
  async observe() {
    await this.healthy();
    const visibleTargets: string[] = [];
    for (const key of Object.keys(this.profile.targets)) if (await this.visible(key)) visibleTargets.push(key);
    ...
    const screen = visibleTargets.map(key => `${key}: ${this.profile.targets[key]!.name}`).join('\n');
    return {screen,visibleTargets};
  }
  async act(a: ActionSpec, inputs: Record<string, unknown>, outputs: Capability['outputs']) {
    if (this.owner !== 'automation') throw new RunError('CONTROL_NOT_OWNED', 'Automation does not own this session');
    ...
    if (await l.count() !== 1) throw new RunError('AMBIGUOUS_TARGET', 'Expected exactly one target');
    if (a.type === 'fill') {
      ...
      if (!field || this.profile.outputTargets[a.output] !== a.target) throw new RunError('CONTRACT_MISMATCH', 'Invalid output binding');
      const text = (await l.innerText()).trim();
      const value = field.type === 'number' ? (/^-?\d+(\.\d+)?$/.test(text) ? Number(text) : NaN) : text;
      if (value === '' || (typeof value === 'number' && !Number.isFinite(value))) throw new RunError('OUTPUT_PARSE_FAILED', 'Output does not match its declared type');
  ...
  async failureEvidence() {
    ...
    const state = await this.observe().catch(() => ({screen:'Observation unavailable after a boundary failure',visibleTargets:[]}));
    ...
    await this.evidence.json('failure-dom.json', {kind:'sanitized_dom_snapshot', ...state, owner:this.owner,controls});
