...
const {values,positionals} = parseArgs({allowPositionals:true,options:{
  ...
  goal:{type:'string',default:'Look up the supplied member and read their available savings balance and currency.'},
  target:{type:'string'},
...
try {
  ...
  if (!['discover','replay'].includes(mode ?? '')) throw new RunError('USAGE','Use discover or replay');
  ...
  const inputs = z.record(z.string(),z.union([z.string(),z.number()])).parse(JSON.parse(values.inputs));
  const evidence = new Evidence(values.evidence,Object.values(inputs));
  surface = new BrowserSurface(profile,evidence);
  const planner = mode === 'discover' ? new (await import('./planner.js')).ApiPlanner(evidence) : undefined;
  await surface.launch(values.headed || values.human,values.scenario);
  runner = new Runner(surface,evidence,values.human);
  await runner.handoff.start();
  let result;
  if (mode === 'discover') {
    const contract = Artifact.pick({name:true,description:true,inputs:true,outputs:true}).parse(JSON.parse(await readFile(values.contract,'utf8')));
  ...
  const displayed = JSON.stringify(values['show-outputs'] ? result : evidence.sanitize(result),null,2);
  console.log(displayed);
  console.log(`Evidence: ${evidence.dir}`);
  process.exitCode = result.status === 'failure' ? 1 : 0;
} catch (error) {
  console.error(error instanceof RunError ? `${error.code}: ${error.message}` : 'Configuration or startup failed. Check file paths, browser installation, and local app availability.');
  process.exitCode = 1;
} finally {
